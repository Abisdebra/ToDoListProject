...
document.getElementById("close-warning")?.addEventListener("click", () => {
  let warningOutput = <HTMLDivElement>document.getElementById("warning");
  warningOutput.style.display = "none";
});

const getUsernameWithEmail = async (email: string) => {
...
const processFormSignUp = async () => {
  ...
  let warning = <HTMLHeadingElement>document.getElementById("warning-text");
  let warningOutput = <HTMLDivElement>document.getElementById("warning");
  ...
  for (let i = 0; i < usernameInput.length; i++) {
    ...
    if (!((code >= 65 && code <= 90) || (code >= 97 && code <= 122))) {
      const errorMessage =
        "Username must only contain lowercase and uppercase letters";
      warning.textContent = errorMessage;
      warningOutputStyle.display = "block";
      throw new Error(errorMessage);
    }
  ...
  if (passwordInput.length < 8 || Object.keys(typesOfCharacter).length < 4) {
    const errorMessage =
      "Password must have a minimum length of 8 and must contain at least one lowercase, uppercase and special character";
    warning.textContent = errorMessage;
    warningOutputStyle.display = "block";
    throw new Error(errorMessage);
  }
  ...
  try {
    ...
    if (response.status === SUCCESS_STATUS) {
      ...
      const errorMessage =
        "Email already registered, did you mean to sign in instead?";
      warning.textContent = errorMessage;
      warningOutputStyle.display = "block";
      throw new Error(errorMessage);
    }
...
const retrieveUserTasks = async (): Promise<Task[]> => {
  const username = getUsernameFromWindows();
...
const generateTableRows = (data: Task[]) => {
  const tableBody = <HTMLTableElement>document.getElementById("table-body");
...
const generateInitialTable = async () => {
  const userTasks: Task[] = await retrieveUserTasks();
  generateTableRows(userTasks);
};

generateInitialTable();

const allSection = document.getElementById("all-section") as HTMLElement;
const activeSection = document.getElementById("active-section") as HTMLElement;
const completedSection = document.getElementById(
  "completed-section"
) as HTMLElement;

const setStyleOfSections = (selectedSection: HTMLElement) => {
...
const addNewTask = async () => {
  let newTaskInput = <HTMLInputElement>document.getElementById("add-task");
  let newTaskPriority = <HTMLSelectElement>(
    document.getElementById("select-priority")
  );
...
const deleteAction = async (username: string, taskId: string) => {
  ...
  const updatedTasks: Task[] = await retrieveUserTasks();
  generateTableRows(updatedTasks);
};

const completeAction = async (username: string, taskId: string) => {
...
const editAction = async (username: string, taskId: string) => {
